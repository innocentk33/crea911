...
@Component({
  selector: "app-register-creatif",
  templateUrl: "./register-creatif.component.html",
  styleUrls: ["./register-creatif.component.less"]
})
export class RegisterCreatifComponent implements OnInit, OnDestroy {
  ...
  sendData() {
    if (
      this.infosCreatifForm.valid &&
      this.infosCreatifForm.controls.password.value ==
        this.infosCreatifForm.controls.confirm_password.value
    ) {
      this.dialogRef = this.previewDialog.open();

      this.serviceCrea911
        .registerCreatif(this.infosCreatifForm.value)
        .subscribe(
          response => {
